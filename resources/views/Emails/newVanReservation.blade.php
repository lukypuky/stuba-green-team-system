    ...
    <div style="background-color: #eaeaea; text-align: -webkit-center; padding: 35px 0;">
        <div style="padding: 20px; background-color: white; border-radius: 25px; width: 576px;">
            ...
            <div style="text-align: left">
                @if ($screen == 1)
                    <div>
                        Používateľ {{ $name }} zadal novú požiadavku pre rezerváciu dodávky v čase od
                        {{ $reservationFrom }} do {{ $reservationTo }}.
                    </div><br>
                @else
                    <div>
                        Používateľ {{ $name }} zmazal požiadavku pre rezerváciu dodávky v čase od
                        {{ $reservationFrom }} do {{ $reservationTo }}.
                    </div><br>
                @endif
