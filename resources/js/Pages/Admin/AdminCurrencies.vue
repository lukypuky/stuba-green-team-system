        ...
        <div class="container page-container">
            <div class="mx-auto grid grid-cols-12 gap-4 p-1">
                ...
                <div class="col-span-12 sm:col-span-10">
                    ...
                    <div class="m-2">
                        <Link :href="route('dashboard-get-currency-store-page')">Pridať záznam</Link>
                    </div>
                    <div>
                        <table class="min-w-full border-collapse block md:table">
                            <thead class="block md:table-header-group">
                                <tr class="border border-grey-500 md:border-none block md:table-row absolute -top-full md:top-auto -left-full md:left-auto  md:relative ">
                                    ...
                                    <th class="bg-gray-600 p-2 text-white font-bold md:border md:border-grey-500 text-left block md:table-cell">Symbol</th>
                                    <th class="bg-gray-600 p-2 text-white font-bold md:border md:border-grey-500 text-left block md:table-cell">Skratka</th>
                                </tr>
                            </thead>
                            <tbody class="block md:table-row-group">
                                <tr class="bg-gray-300 border border-grey-500 md:border-none block md:table-row"
                                    v-for="(currency, index) in this.currencies.data" :key="index">
                                    <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell"><span class="inline-block w-1/3 md:hidden font-bold">Akcie</span>
                                        <div class="flex">
                                            <div class="pr-2">
                                                <Icon name="update" width="20" @click.prevent="getDetail(currency.id)"/>
                                            </div>
                                            <div class="pl-2">
                                                <Icon name="delete" width="20" @click.prevent="deleteModal(currency.id)"/>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell"><span class="inline-block w-1/3 md:hidden font-bold">Symbol</span>
                                        {{ currency.currency_symbol }}
                                    </td>
                                    <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell"><span class="inline-block w-1/3 md:hidden font-bold">Skratka</span>
                                        {{ currency.shortcut }}
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    <div>
                        <Pagination :links="this.currencies.links"/>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {            
            ...
            deleteObject(){
                Inertia.post(route('dashboard-delete-currency'), this.deletedObjectId);
                this.closeDeleteModal();
            },
            getDetail(param){
                var request = {id: param};
                Inertia.get(route("dashboard-currency-detail", request));
            }
        },
        computed: {
            showAlert() {
