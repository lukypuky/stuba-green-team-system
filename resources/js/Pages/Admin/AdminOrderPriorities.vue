                ...
                <div class="col-span-12 sm:col-span-10">
                    ...
                    <div class="mb-3">
                        <div class="leading-10">
                            <Link :href="route('dashboard-get-order-priority-store-page')" class="buttons">Pridať záznam</Link>
                        </div>
                    </div>
                    <div class="overflow-auto rounded-lg shadow hidden md:block">
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <tr class="bg-white" v-for="(orderPriority, index) in this.orderPriorities.data" :key="index">
                                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                                        <div class="flex">
                                            <div class="pr-2">
                                                <Icon name="update" width="20" @click.prevent="getDetail(orderPriority.id)"/>
                                            </div>
                                            <div class="pl-2">
                                                <Icon name="delete" width="20" @click.prevent="deleteModal(orderPriority.id)"/>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                                        {{ orderPriority.order_priority_title }}
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <div class="grid grid-cols-1 gap-4 md:hidden mb-2 text-sm" v-for="(orderPriority, index) in this.orderPriorities.data" :key="index">
                    ...
                    <div>
                        <Pagination :links="this.orderPriorities.links"/>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {            
            ...
            deleteObject(){
                Inertia.post(route('dashboard-delete-order-priority'), this.deletedObjectId);
                this.closeDeleteModal();
            },
            getDetail(param){
                var request = {id: param};
                Inertia.get(route("dashboard-order-priority-detail", request));
            }
        },
        computed: {
            showAlert() {
